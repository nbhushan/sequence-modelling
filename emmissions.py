...
class Gaussian:
    ...
    def sample(self, stateseq):
        ...
        """        
        if self.dim ==1:
            return np.random.normal(self.mu[:,stateseq].flatten(),np.sqrt(self.covar[stateseq,:,:]).flatten())        
        else:
            return np.random.multivariate_normal(self.mu[:,stateseq].flatten(), self.covar[stateseq,:,:].flatten())
