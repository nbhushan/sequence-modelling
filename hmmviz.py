...
def view_postduration(ax,obs, path, mean, reslist, ranknlist, seq):
    ...
    for k,v in enumerate(m):
        t = z==k    
        ax.plot(r[t],v*np.ones((t.sum(),),float),'.', c=clr[::-1][k]\
                ,label='State {0}: {1:g}'.format(k,v))
    ...
    for k in range(1,len(m)-1):
        ...
        for label, x, z in zip(np.char.mod('%.2f', res[k][c]), rankn[k][c]+1,[mean.flatten()[k]]*res.shape[1]):
            ax.annotate(
                label, 
                xy = (x, z), xytext = (-20, 20),
...
def view_statedurations(fc, path, K):
    ...
    n,binsw,patches = aw.hist(lengths[0],normed=1, histtype='stepfilled',label='active')
    n,binsx,patches = ax.hist(lengths[1],normed=1, histtype='stepfilled',label='idle')
    n,binsy,patches = ay.hist(lengths[2],normed=1, histtype='stepfilled',label='sleep')
    n,binsz,patches = az.hist(lengths[3],normed=1, histtype='stepfilled',label='deep sleep')    
    aw.legend()
    ax.legend()
    ay.legend()
    az.legend()
