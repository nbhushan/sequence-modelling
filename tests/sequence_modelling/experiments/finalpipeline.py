...
logging.basicConfig(
    filename="C:\\Local\\thesis\\experiments\\SAsearch\\dauphinepipeline1.log",
    level=logging.DEBUG,
)
...
def _generatedata(N, nseq, dim, tau, zeta, eta):
    ...
    tau = tau
    p, zeta, eta, D = 0.001, zeta, eta, np.max(tau)

    # Create an  HMM and Gaussian emmission object to sample data
    _emmissionModel = emissionplus.Gaussian(
        mu=np.array([[1254.39, 429.88, 254.60, 52.36]]),
        var=np.array([[[46560.05]], [[2500.68]], [[734.04]], [[159.58]]]),
        tau=tau,
    )
...
def fitHMM(obs):
    ...
    _emmissionModel = Gaussian(
        mu=np.array([[1254.39, 429.88, 254.60, 52.36]]),
        covar=np.array([[[46560.05]], [[5500.68]], [[2734.04]], [[159.58]]]),
    )
    hmmmodel = StandardHMM(A, _emmissionModel)
    # Fit the model to the data and print results
    start_time = time.time()
    newloglikehood, ll, duration, rankn, res = hmmmodel.hmmFit(
        obs, maxiter=10, epsilon=1e-6, debug=True
    )
    end_time = time.time()
    logging.info(
        "Time to estimate initial HMM model : %.5f s " % (end_time - start_time)
    )
...
def fitQDHMM(obs, initmodel, tau):
    ...
    _emmissionModel = emissionplus.Gaussian(
        mu=initmodel.O.mu, var=initmodel.O.covar, tau=tau
    )
    """
    #Dauphine emission model
    _emmissionModel = \
    ...
    end_time = time.time()
    logging.info(
        "Time taken to estimate QDHMM parameters (s) : %.5f s" % (end_time - start_time)
    )
...
if __name__ == "__main__":
    ...
    # Sample Data
    obs = _generatedata(
        N=1000, nseq=1, dim=1, tau=np.array([0, 6, 10]), zeta=0.65, eta=0.966
    )
