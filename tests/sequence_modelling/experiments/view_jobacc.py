...
def data():
    ...
    with open("dauphine_1107to1108_jobbacc.csv") as u:
        ...
        Ljob = tuple(
            (
                tstamp(datetime.strptime(x[0], "%Y-%m-%d %H:%M:%S")),
                tstamp(datetime.strptime(x[1], "%Y-%m-%d %H:%M:%S")),
                int(x[2]),
            )
            for x in ur
        )
...
def display(data, ax, mibshift=0, jobshift=0):
    # pdb.set_trace()
    def dconv(x, d0=-tstamp(datetime(1, 1, 1)), daysecs=24 * 3600.0):
        return (d0 + x) / daysecs
    ...
    ax.bar(
        dconv(ajob[:, 0] - jobshift),
        len(ajob) * [600],
        dconv(ajob[:, 1] - ajob[:, 0], 0),
        0,
        color="c",
    )
