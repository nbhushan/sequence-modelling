...
def test():
    ...
    emmissionModel = Gaussian(
        mu=np.array([[3902.85, 2332.44, 809.70, 258.08, 76.70]]),
        covar=np.array([[[341.05]], [[215.68]], [[189.58]], [[90.32]], [[47.44]]]),
    )
    ...
    likelihood, ll, durationlist, ranknlist, reslist = wintermodel.hmmFit(
        obs, 100, 1e-6, debug=True
    )
    path = [None] * numseq
    ...
    # ==============================================================================
    feeder_power = np.column_stack(
        (feeder_power_A, feeder_power_B, feeder_power_C, feeder_power_D)
    )
