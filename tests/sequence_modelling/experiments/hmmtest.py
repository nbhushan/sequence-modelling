...
def test():
    ...
    tau = np.array([0, 8, 14 ])    
    p, zeta, eta,  D = .001, .45, .97, np.max(tau)     
    
    # Create an  QDHMM and Gaussian emmission object to sample data       
    emmissionModel = (
        emissionplus.Gaussian(mu = np.array([[532.23915 , 86.69044, 45.2,  26.552]]), \
    ...
    for seq in range(numseq):
        ...
        pp.savefig()
        fb=figure()
        viz.view_postduration(fb.add_subplot(111), obs, path, trainmodel.O.mu, res, rankn, seq)
        fb.tight_layout()
        pp.savefig()
    fc=figure()
