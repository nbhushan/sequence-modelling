...
def test():
    ...
    emmissionModel = Gaussian(
        mu=np.array([[1054.39, 329.88, 154.60, 52.36]]),
        covar=np.array([[[46560.05]], [[2050.68]], [[2734.04]], [[159.58]]]),
    )
    ...
    obs[0] = np.loadtxt("dauphine11071309h15h.csv", delimiter=",", skiprows=1)[
        np.newaxis, :
    ]
    ...
    likelihood, ll, durationlist, ranknlist, reslist = dauphinemodel.hmmFit(
        obs, 20, 1e-6, debug=True
    )
    path = [None] * numseq
    ...
    viz.view_postduration(
        fb.add_subplot(111), obs, path, dauphinemodel.O.mu, reslist, ranknlist, seq=0
    )
    fb.tight_layout()
    pp.savefig()
    fc = figure()
