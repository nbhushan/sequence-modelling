...
class QDHMM:
        ...
        for n in range(N):
            ...
            if d==0:
                ...
                d = min(d+1,self.D-1) if \
                    np.random.multinomial(1,[(1-self.eta),self.eta])[0]==0 else 0
    ...
    def gammaKsi(self, B):
        ...
        for n in range(N-1):   
            ...
            _temp_eta[n] =1.0-(temp_array[0,1]+ np.sum(temp_array.data[temp_array.indptr[0]:temp_array.indptr[1]])) 
        logger.debug('Time to compute (ksi) zera and eta arrays : %.5f s '
                                                        % (time.time()-to))                 
        return (loglikelihood/N, Gamma, np.sum(_temp_zeta), 
                                                np.sum(_temp_eta) )
        
     
    def qdhmmFit(self, obs, maxiter=50 , epsilon=1e-5, debug=False, metaheuristic= 'local'):
        ...
        for iteration in range(maxiter):   
            ...
            for seq,obsseq in enumerate(obs):                                     
                ...
                logger.debug('E step..' )
                loglikelihoodlist[seq], Gammalist[seq], \
                    zetalist[seq], etalist[seq]= self.gammaKsi(B)         
