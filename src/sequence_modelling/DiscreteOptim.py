...
def objective(taus, K, obs, weights):
    ...
    for tau in taus:
        ...
        energy.append((np.sum(np.sum(weights[0:tau[0]+1,:],0)) *np.log(var[0,:,:])+
        np.sum(np.sum(weights[tau[0]+1:tau[1]+1,:],0)) *np.log(var[1,:,:]) + 
                               np.sum(np.sum(weights[tau[1]+1:tau[2]+1,:],0)) *np.log(var[2,:,:]) + 
                               np.sum(np.sum(weights[tau[2]+1:,:],0)) *np.log(var[3,:,:]))[0][0])
    return energy              
...
def new_objective(tau, K, obs, weights):
    ...
    for k in range(0,K):
        ...
        obs_bar = obs - mean[:,k][:,np.newaxis]
        var[k,:,:] = np.dot( normalizer[k,:]*obs_bar , obs_bar.T)      
    energy = ((np.sum(np.sum(weights[0:tau[0]+1,:],0)) *np.log(var[0,:,:])+
    np.sum(np.sum(weights[tau[0]+1:tau[1]+1,:],0)) *np.log(var[1,:,:]) + 
                           np.sum(np.sum(weights[tau[1]+1:tau[2]+1,:],0)) *np.log(var[2,:,:]) + 
                           np.sum(np.sum(weights[tau[2]+1:,:],0)) *np.log(var[3,:,:]))[0][0])
    return energy    
...
def checkpopulation(population, D):
    ...
    '''
    good_population = [n for n in population if not(n[1] >=n[2]) and n[1]>=1 and (n>=0).all() and (n<D-1).all()]  
...
def generatepopulation( taus, weights, obs, number, step, D):
    ...
    population = [n for n in c if not(n[1] >=n[2]) and n[1]>=1 and (n>=0).all() and (n<D-1).all()]      
    return population
...
def geneticalgorithm(obj, obs, maxiter, weights):
    ...
    while count < maxiter: 
        ...
        if len(population)>1:
            ...
            strongest_tau1 = population[np.argsort(fitness)[np.random.randint(0,2,1)]][1]
            strongest_tau2 = population[np.argsort(fitness)[np.random.randint(0,2,1)]][2]
