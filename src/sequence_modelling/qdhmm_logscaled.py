...
class HMMPlus:
    ...
    def sample(self, dim , N):
        ...
        for n in range(N):
            ...
            if d==0:
                ...
                d = min(d+1,self.D-1) if \
                    np.random.multinomial(1,[(1-eta),eta])[0]==0 else 0
        return obs,zes
    ...
    def gammaKsi(self, logB):
        ...
        logger.debug('Time to compute (ksi) zera and eta arrays : %.5f s '% (time.time()-to))                 
        return (loglikelihood/N, logGamma, logsumexp(temp_zeta), \
                                                logsumexp(temp_eta) )
        
     
    def hmmFit(self, obs, maxiter = 50 , epsilon = 0.000001,\
        ...
        for iteration in range(maxiter):   
            ...
            to = time.time()
            self.logA  =self.buildTransmat()
            logger.debug('Time to create sparse transition matrix : %.5f s '% (time.time()-to))
            ...
            for seq,obsseq in enumerate(obs):                                     
                ...
                logger.debug('Time to create emission distribution matrix : %.5f s '% (time.time()-to))                
                logger.debug('E step..' )
                loglikelihoodlist[seq], logGammalist[seq], \
                    logzetalist[seq], logetalist[seq]= self.gammaKsi(logB)
    ...
    def displayviterbi(self,ax,y,z,m):
        ...
        for k,v in enumerate(m):
            t = z==k        
            ax.plot(r[t],v*np.ones((t.sum(),),float),'.'\
